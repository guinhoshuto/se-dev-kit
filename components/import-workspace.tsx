...
export function ImportWorkspace() {
  ...
  async function create(snapshot: WidgetSnapshot) {
    ...
    try {
      ...
      if (creationKey) headers['X-Studio-Key'] = creationKey;
      const response = await fetch('/api/v1/projects', {method: 'POST', headers, body: JSON.stringify(snapshot)});
  ...
  return <div className="app-shell">
    ...
    <main className="import-layout">
      <section className="import-main">
        ...
        <label className="field-control spaced" htmlFor="creation-key"><span>Workspace creation key</span><input id="creation-key" type="password" autoComplete="off" spellCheck={false} value={creationKey} onChange={event => setCreationKey(event.target.value)} aria-describedby="creation-key-help" /></label>
        <p id="creation-key-help" className="muted fine-print">Optional. Required only when the workspace owner has protected project creation. Not saved by the Studio.</p>
